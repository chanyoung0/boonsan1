...
public class UnderwritingService {
    ...
    // 수동심사 유형별 감점값 반환
    public int getManualUnderwritingAdjustment(String type) {
        switch (type) {
            case "1": return -10;
            case "2": return  -5;
            case "3": return  -3;
    ...
    // 심사결과 판정 (점수 기준)
    public String determineResult(int score) {
    ...
    public boolean needsCoinsurance(int score) {
        return score < 70;
    }

    // 재보험 처리 필요 여부 판단 (자사 보유한도 초과)
    public boolean needsReinsurance(long insuranceAmount) {
        return insuranceAmount > REINSURANCE_THRESHOLD;
    }

    // Underwriting + UnderwritingResult 객체 생성 및 저장
    public void saveUnderwritingResult(String empName, int score, String finalResult, boolean coinsuranceRecommended) {
    ...
    public InsuranceApplication createAndSaveApplication(String name, long insuranceAmount, String appliedCondition) {
    ...
    // Contract 생성 및 저장
    public void createAndSaveContract(String policyNo, InsuranceApplication application) {
    ...
    private boolean isDrinkingNone(String val) {
    ...
    private double parseBmi(String s) {
