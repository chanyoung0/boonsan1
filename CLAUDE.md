...
## Service 구성 원칙 (유스케이스 단위)

Service 클래스는 **도메인 클래스 단위가 아니라 유스케이스(업무 흐름) 단위로 묶는다.**

- 도메인 클래스는 데이터(명사) 기준으로 세분화되어 37개다.
- Service 클래스는 유스케이스(동사) 기준으로 통합되어 12개다 (유스케이스 다이어그램의 기능 수에 맞춤).
- 따라서 하나의 Service가 여러 도메인을 다루는 N:1 구조가 정상이다. 1:1로 만들지 마라.

예) `UnderwritingService` 하나가 Underwriting, UnderwritingHistory, UnderwritingRequest, UnderwritingResult, InsuranceApplication, Coinsurance, Coinsurer, Reinsurance를 함께 다룬다. (보험청약 심사라는 한 유스케이스가 이 도메인들을 모두 사용하기 때문)

유스케이스 ↔ Service 대응:

| 유스케이스 | Service |
|---|---|
| 1. 보험청약 심사 | UnderwritingService |
| 2. 배서 관리 | EndorsementService |
| 3. 부활 관리 | ReinstatementService |
| 4. 분납/수금 관리 | PaymentCollectionService |
| 5. 만기계약 관리 | MaturityContractService |
| 6. 사고 접수 | AccidentReportService |
| 7. 손해조사 | DamageInvestigationService |
| 8. 상품 개발 | InsuranceProductService |
| 9. 제지급금 관리 | PayoutService |
| 10. 보상평가 관리 | CompensationEvaluationService |
| 11. 협력업체 관리 | PartnerService |
| 12. 구상 처리 | SubrogationService |

새 Service를 만들 때는 "이게 어느 유스케이스에 속하는가"를 먼저 확인하고, 기존 유스케이스에 속하면 새 클래스를 만들지 말고 해당 Service에 메서드를 추가한다.
